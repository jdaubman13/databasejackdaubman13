...
FROM fortress1 
WHERE (MOB_NAME) IN (SELECT MOB_NAME FROM FORTRESS2);
...
-- Question 5
SELECT MOB_NAME, SPAWNS, BIOME_ID 
FROM fortress1 
WHERE SPAWNS > (SELECT AVG(biome_avg) FROM (SELECT AVG(SPAWNS) AS biome_avg FROM fortress1 GROUP BY BIOME_ID) biome_averages);
